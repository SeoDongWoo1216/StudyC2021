...
이 개행문자가 이후의 입력에 영향을 줄 수 있으므로 getchar 함수를 추가로 호출하여 버퍼에 남아 있던 개행 문자를 제거해야한다. <br>
```C
scanf("%d", &num);  // 학번 입력
getchar();          // 버퍼에 남아있는 개행 문자 제거
```
<br>
버퍼가 비어있으면 다시 키보드로부터 입력을 시도하여 학점을 제대로 입력할 수 있게 된다. <br>
버퍼의 개행 문자를 제거하지 않으면 버퍼에 있는 개행 문자를 11행에서 호출되는 getchar 함수가 가져간다. <br>
따라서 학점을 추가로 입력받지 못하고 프로그램이 계속 진행되며 grade에 저장된 개행 문자는 출력문으로 넘어가게되어 줄이 바뀌게 된다. <br>

```C
grade = getchar();   
printf("학번 : %d, 학점 : %c", num, grade); 
```
만약 scanf 다음줄에 getchar()가 없었으면 결과는 다음과 같다.
```C
학번 입력 : 314 (엔터)
학점 입력 : 학번 : 315, 학점 :
(커서)
```
학점을 입력할 수 없고 학점을 출력할 곳에서 줄이바뀌게되어 커서가 다음줄로 내려오는 런타임 에러가 발생하게된다. <br>
